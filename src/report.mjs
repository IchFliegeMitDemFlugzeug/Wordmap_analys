...
export function generateReport(db, runDir) {
  const queryHeaders = ['id', 'query', 'normalized', 'depth', 'score', 'relevance_class', 'relevance_reason', 'recursive_eligible', 'deep_eligible', 'expansion_status', 'skip_reason', 'manual_seed', 'brand_seed', 'status', 'root_seed', 'top_total_count', 'discovered_count_max', 'dynamics_status', 'regions_status', 'serp_status'];
  ...
  ${section('Manual seeds', queries.filter((query) => query.manual_seed))}${section('CORE', priorityRows('core'))}${section('ADJACENT', priorityRows('adjacent'))}${section('Наиболее популярные BROAD', priorityRows('broad', 40))}${section('Примеры NOISE', priorityRows('noise', 25))}${section('Брендовые', queries.filter((query) => query.brand_seed))}${section('Кластеры', data.clusters, ['id', 'name', 'query', 'score'])}${section('SERP domains', data.serp, ['query', 'position', 'domain', 'url'])}${section('Dynamics', data.dynamics, ['query', 'date', 'count', 'share'])}${section('Regions', data.regions, ['query', 'region_name', 'count', 'share', 'affinity_index'])}${section('Relations / происхождение запросов', data.relations.slice(0, 500), ['parent_query', 'child_query', 'relation_type', 'count'])}</body></html>`;
