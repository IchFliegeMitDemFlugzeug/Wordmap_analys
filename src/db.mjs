...
CREATE TABLE IF NOT EXISTS api_calls (id INTEGER PRIMARY KEY, api TEXT, method TEXT, query_id INTEGER, http_status INTEGER, started_at TEXT, finished_at TEXT, success INTEGER, error TEXT);
CREATE INDEX IF NOT EXISTS idx_queries_queue ON queries(status,manual_seed DESC,score DESC,depth,id);
CREATE INDEX IF NOT EXISTS idx_calls_quota ON api_calls(api,finished_at);
...
export function addQuery(db, query, data = {}) {
  const now = new Date().toISOString(), normalized = normalizeQuery(query);
  db.prepare(`INSERT INTO queries(query,normalized,depth,score,manual_seed,brand_seed,status,root_seed,first_seen_at,last_seen_at) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(normalized) DO UPDATE SET last_seen_at=excluded.last_seen_at, manual_seed=MAX(manual_seed,excluded.manual_seed), brand_seed=MAX(brand_seed,excluded.brand_seed), score=MAX(score,excluded.score), depth=MIN(depth,excluded.depth)`).run(query, normalized, data.depth ?? 0, data.score ?? 0, data.manualSeed ? 1 : 0, data.brandSeed ? 1 : 0, data.status ?? 'queued', data.rootSeed ?? query, now, now);
  return db.prepare('SELECT * FROM queries WHERE normalized=?').get(normalized);
}
export function recoverProcessing(db) { return db.prepare("UPDATE queries SET status='queued' WHERE status='processing'").run().changes; }
export function quotaState(db, now = Date.now()) {
  const cutoff = new Date(now - 3600000).toISOString();
  const rows = db.prepare("SELECT finished_at FROM api_calls WHERE api='wordstat' AND finished_at>=? ORDER BY finished_at").all(cutoff);
  return { count: rows.length, waitMs: rows.length < 95 ? 0 : Math.max(0, Date.parse(rows[0].finished_at) + 3605000 - now) };
}
